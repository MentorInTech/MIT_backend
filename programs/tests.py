...
class TestModel(TestCase):
    ...
    def test_assign_new_goal(self):
        goal = Program.objects.create(title='A Program', role='MTR', score=10, profile=self.test_profile)
        self.assertEqual(len(self.test_profile.program_set.all()), 1)
        self.assertEqual(self.test_profile.program_set.all()[0], goal)
...
class TestAPI(APITestCase):
    def setUp(self):
        ...
        self.goal_url = reverse('program_list')
